...
class Context(object):
    ...
    def __init__(self, manager, rpcid):
        ...
        self.rpcid = rpcid
        self.result = None
        self.result_ev = asyncio.Event(loop=self.manager._event_loop)
    #enddef
...
def agentcall(script):
    '''Define a function running on the agent that can be called from the host.

    Only meant to be used within a `BaseRPC` subclass definition. The string
    argument acts as the source of the function.
    ...
    '''
    return RPCall(CallDirection.AGENT, script)
#enddef
...
def hostcall(func):
    ...
    '''
    return RPCall(CallDirection.HOST, func)
#enddef
    ...
    def __new__(meta, name, bases, dct):
        ...
        for (n, v) in dct.items():
            if not isinstance(v, RPCall):
                ...
                continue
            #endif
            if v.direction == CallDirection.AGENT:
                agent_calls_info[n] = {
                    "script" : v.args[0]
                }
            elif v.direction == CallDirection.HOST:
                host_calls_info[n] = {
                    "func" : v.args[0]
                }
            #endif
...
class BaseRPC(scribe.Scribe, metaclass=RPCMeta):
    ...
    def on_rpc_message(self, payload, data):
        ...
        if rpctype == "host_call":
            ...
            self.ctxman.set_context_result(rpcid, payload["result"])
        else:
